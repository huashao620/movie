...
<div>
  <el-form :inline="true" :model="queryParam">
    <el-form-item label="收藏用户">
      <el-select size="small" v-model="queryParam.userId" filterable clearable width="370px">
        <el-option v-for="item in userList" :key="item.id" :label="item.name" :value="item.id" />
      </el-select>
    </el-form-item>
    <el-form-item label="收藏电影">
  ...
  <el-table :data="tableData" border style="width: 100%">
    <el-table-column prop="user.name" label="收藏用户" align="center" />
    <el-table-column prop="movie.name" label="收藏电影" align="center" />
  ...
  ></el-pagination>
  <el-dialog append-to-body :close-on-click-modal="false" :visible.sync="editDialogFlag" title="编辑收藏记录" width="600px">
    <el-form ref="saveForm" inline :model="saveForm" :rules="rules" size="small" label-width="120px">
      ...
      <el-form-item label="收藏电影" prop="movieId">
...
export default {
  data() {
    return {
      ...
      rules: {
        userId: [
          { required: true, message: '收藏用户不能为空', trigger: 'change' }
        ],
        movieId: [
          { required: true, message: '收藏电影不能为空', trigger: 'change' }
        ],
      },
      loginer: {}
    }
  ...
  methods: {
    ...
    save() {
      this.$refs.saveForm.validate(res => {
        if (res) {
          this.$axios({
            url: this.saveForm.id ? '/api/movie-collect/update': '/api/movie-collect/add',
            method: 'post',
            data: this.saveForm
          }).then(res => {
            if (res.data.code === 0) {
    ...
    toDelete(row) {
      this.$confirm('确定删除吗?', '提示', {
        ...
        this.$axios({
          url: '/api/movie-collect/delete?id=' + row.id,
          method: 'delete'
        }).then(res => {
          if (res.data.code === 0) {
